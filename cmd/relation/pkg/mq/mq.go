...
var MysqlManager *mysql.RelationManager
var AddActor *Actor
var DelActor *Actor
...
func InitMq() {
	...
	AddActor, err = NewActor(amqpConn, "follow_add")
	if err != nil {
		klog.Fatal("cannot create add actor", err)
	}
	// start consumer thread
	go AddActor.Consumer(context.Background())
	DelActor, err = NewActor(amqpConn, "follow_del")
	if err != nil {
...
func (a *Actor) FollowAdd(msgs <-chan amqp.Delivery) {
	for d := range msgs {
		...
		if err := MysqlManager.AddFollow(int64(UserId), int64(ToUserId)); err != nil {
...
func (a *Actor) FollowDel(msgs <-chan amqp.Delivery) {
	for d := range msgs {
		...
		if err := MysqlManager.DelFollow(int64(UserId), int64(ToUserId)); err != nil {
